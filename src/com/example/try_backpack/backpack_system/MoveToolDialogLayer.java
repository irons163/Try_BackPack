...
public class MoveToolDialogLayer extends DialogLayer{
	ViewLayer viewLayer;
	ViewGroupLayer viewGroupLayer;

	public MoveToolDialogLayer(Context context, float x, float y) {
		super();
		setPosition(x, y);
		
		viewLayer = new ViewLayer(context, 0, 0, false);
		...
		viewLayer.setView(editText);
		viewGroupLayer = new ViewGroupLayer(context, 0, 0, false);
//		viewGroupLayer.setViewGroup(new RelativeLayout(context));
		viewGroupLayer.addChild(viewLayer);
		
		addChild(viewGroupLayer);
